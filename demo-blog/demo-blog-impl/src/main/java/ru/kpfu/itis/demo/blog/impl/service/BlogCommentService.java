...
public class BlogCommentService implements CommentService {
    ...
    public Page<CommentDTO> findAll(Pageable pageable) {
        return commentRepository.findAll(pageable).map(commentEntity ->
                modelMapper.map(commentEntity, CommentDTO.class));
    }


    @Override
    public Optional<CommentDTO> findById(Long aLong) {
        return commentRepository.findById(aLong).map(commentEntity ->
                modelMapper.map(commentEntity, CommentDTO.class));
    }
